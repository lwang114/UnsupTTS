...
setups="matched" # XXX "matched unmatched"
...
if [ $stage -le 0 ] && [ $stage -ge 0 ]; then
    ...
    find $timit_root/{TRAIN,TEST} -name "*.WAV" > $tgt_dir/all_sph.flist
    ...
    find $timit_root/{TRAIN,TEST} -name "*.PHN" > $tgt_dir/all_phn60.flist
    while read line; do
      if [ ! -f $line ]; then 
...
if [ $stage -le 2 ] && [ $stage -ge 2 ]; then
    for s in $setups; do
      bash scripts/prepare_audio.sh $tgt_dir/$s $tgt_dir/$s/feat $model
