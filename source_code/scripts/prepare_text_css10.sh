...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
    ...
    echo "Transcription type: ${trans_type}"
    ...
    elif [ $trans_type = phn ]; then 
        echo "Use phoneme transcripts"
        for split in train valid; do
            for suffix in wrd phn; do
                if [ ! -f $target_dir/char_${split}.${suffix} ]; then
                    cp $target_dir/${split}.${suffix} $target_dir/char_${split}.${suffix}
                fi
            done  
        done

        for split in train valid; do
            python scripts/g2p_wrd_to_phn_language_net.py \
                --wrd_path $target_dir/char_${split}.wrd \
                --in_path $target_dir/words.txt \
                --out_name $target_dir/${split} \
                --phoneticsaurus $phoneticsaurus \
                --g2p_models_dir ${g2p_models_dir} \
                --lang ${lg}
        done

        for split in train valid; do
            cp $target_dir/char_${split}.wrd $target_dir/${split}.wrd
